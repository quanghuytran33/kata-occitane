...
class VetController {
	...
	public String showVetList(@RequestParam(defaultValue = "1") int page, Model model) {
		...
		return addPaginationModel(page, vetService.findWithPagination(page), model);

	}

	private String addPaginationModel(int page, VetPaginationView paginated, Model model) {
		model.addAttribute("currentPage", page);
		model.addAttribute("totalPages", paginated.getTotalPages());
		model.addAttribute("totalItems", paginated.getTotalElements());
		model.addAttribute("listVets", paginated.getVets());
		return "vets/vetList";
	}
